...
- **修 Bug 必须先定性:架构问题 vs 实现问题(强制,先定性再动手)**:
  - **架构导致的 bug**(概念缺失、职责错位、同一语义多处实现靠隐式约定对齐、
    状态/口径不唯一):禁止在原结构上打补丁了事 —— 必须先把正确的统一概念立起来
    (或补齐缺失概念),让这一**类**问题在结构上不可能复现,当前现象作为该概念的
    第一个应用顺带修掉;
  - **实现导致的 bug**(概念本身正确,某处写错了):定位到唯一出错点做最小修复,
    不要顺手重构、不要扩大改动面;
  - 判定必须取证:读代码、读依赖源码、读历史提交,禁止凭感觉选边;
  - 案例警示(P2 文本重构):"粗体宽度超行盒"先被误判为实现问题,打了
    "+1 网格像素"的死公式补丁 —— 实际根因是「度量规格未携带渲染样式」的架构缺口。
    正确解法 = 样式并入 MeasureSpec + 测量直接询问渲染器的权威计宽
    (`mc.font.splitter`),并顺手消灭所有自算宽度公式。
