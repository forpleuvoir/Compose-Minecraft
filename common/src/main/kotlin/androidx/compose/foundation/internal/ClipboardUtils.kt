 ...
 */
internal suspend fun ClipEntry.readText(): String? = text
 ...
 */
internal suspend fun ClipEntry.readAnnotatedString(): AnnotatedString? = text?.let { AnnotatedString(it) }

/** Creates a [ClipEntry] from the [AnnotatedString] */
internal fun AnnotatedString?.toClipEntry(): ClipEntry? = this?.let { ClipEntry(it.text) }

/** Returns true if [ClipEntry] has a valid text representation. Otherwise, it returns false. */
internal fun ClipEntry?.hasText(): Boolean = this != null
