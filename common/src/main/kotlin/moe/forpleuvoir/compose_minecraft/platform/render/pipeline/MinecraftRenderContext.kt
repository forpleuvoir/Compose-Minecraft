...
/**
 * 渲染入口(起为纯编排层):把 [MinecraftCanvas] 录制的绘制命令分派给各渲染后端。
 *
 * - 2D 命令 → [CommandDispatcher] → [GuiStateBackend](GUI 状态,原 render 2D 分支);
 * - 3D 命令(携带 layer3D)→ [PerspectiveBackend](CPU 顶点透视变换,,原 render3D 搬移),
