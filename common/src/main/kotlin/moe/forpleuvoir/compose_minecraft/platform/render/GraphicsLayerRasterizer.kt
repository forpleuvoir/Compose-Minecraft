 ...
 * 像素缓冲(0xAARRGGBB),支撑 [androidx.compose.ui.graphics.layer.GraphicsLayer.toImageBitmap]
 * —— 图层内容快照,无渲染上下文依赖,任意线程可调用。
 *
 * 策略(与 GPU 回放同一套几何):
 * - 形状命令(矩形/圆角矩形/圆/椭圆/弧/线/Path/点)统一走 [GeometryTessellator]
 *   三角化,逐三角形重心判定 + coverage 插值(顶点 coverage 是距轮廓的有符号距离,
 *   插值后 clamp 0..1 即得与 GPU 一致的边缘软过渡抗锯齿);
 * - 图片命令经命令矩阵 2D 逆变换采样(dst 矩形内逆映射回局部坐标 → src 采样,
 *   最近邻 / 双线性按 paint.filterQuality);
 * - 渐变矩形用两个三角形 + 顶点色重心插值(渐变方向随矩形旋转,与顶点色语义一致)。
 *
 * 坐标系:命令矩阵把局部坐标 → 记录坐标(图层坐标),光栅化目标缓冲即图层坐标;
 * 命令 clip 同处记录坐标(record 内画布初始单位矩阵,clipRect 换算即图层坐标),
 * 因此 clip 直接用像素坐标判定,无需再变换。
 *
    ...
    fun rasterize(canvas: MinecraftCanvas, width: Int, height: Int): IntArray {
        ...
        for (cmd in canvas.commands()) {
            when (cmd) {
                ...
                is DrawOvalCommand -> tessellate(out, width, height, cmd) { s ->
                ...
                is DrawCircleCommand -> tessellate(out, width, height, cmd) { s ->
                ...
                is DrawArcCommand -> tessellate(out, width, height, cmd) { s ->
                ...
                is DrawLineCommand -> tessellate(out, width, height, cmd) { s ->
                ...
                is DrawPathCommand -> tessellate(out, width, height, cmd) { s ->
                ...
                is DrawPointsCommand -> tessellate(out, width, height, cmd) { s ->
