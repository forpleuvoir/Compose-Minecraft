...
// 仓库内自写的 core/gui_triangles(与 core/gui 同构:POSITION_COLOR + 纯色,
// 保守 GLSL 330,OpenGL / Vulkan 两个渲染后端均可编译)。
//
// 平台适配点:
...
//   管线参数在此显式声明:POSITION_COLOR、TRANSLUCENT 混合、TRIANGLES 拓扑,
//   cull 关闭(三角形绕序不做约束,与 GUI 四边形观感一致);
...
internal object MinecraftGuiTriangles {
     ...
     *
     * 顶点格式 POSITION_COLOR_LINE_WIDTH:LineWidth 属性承载「到轮廓边的屏幕
     * 像素距离」(coverage),片元着色器据此做边缘抗锯齿。
 ...
 * - coverage:与顶点一一对应的「到轮廓边的屏幕像素距离」(LineWidth 属性槽),
 *   片元着色器 clamp 后作边缘抗锯齿;内部三角形为大数(实心);
